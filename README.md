...
6. now that the physical memory page is freed, you need to update the page tables to destroy the mapping. following steps are needed:
  1. set the entire PTE entry to 0. check if the entire page table is free, i.e., if in this page table, every entry's present bit is 0, then we can say this page table is not used at all, and therefore its memory should be freed. call *free_page*() to free this page table.
  2. set the entire PDTE entry to 0. check if the entire PDT table is free, i.e., if in this PDT table, every entry's present bit is 0, then we can say this PDT table is not used at all, and therefore its memory should be freed. call *free_page*() to free this PDT table.
  3. set the entire PDPTE entry to 0. check if the entire PDPT table is free, i.e., if in this page table, every entry's present bit is 0, then we can say this page table is not used at all, and therefore its memory should be freed. call *free_page*() to free this PDPT table.
  4. set the entire PML4E entry to 0. check if the entire PML4 table is free, i.e., if in this page table, every entry's present bit is 0, then we can say this page table is not used at all, and therefore its memory should be freed. call *free_page*() to free this PML4 table.
